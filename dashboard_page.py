...
def render(
    ...
    date_type = st.selectbox(
        "Select date field used in SQL calculations:",
        options=["i.createdAt", "epri.activatedOn", "epri.toBeClearedOnUtc"],
        index=0,
        help="Choose the desired date column for metric period definitions."
    )
    st.caption(
        "The slider controls how far back we look when tagging vendors; "
        "the date field choice will flow to future SQL updates."
    )
    ...
    if run_btn:
        ...
        with st.spinner("Connecting to DB & downloading data…"):
            base_path = pull_module.run(months, date_type=date_type)  # <= your pull.run signature
    ...
    # ───────── Sidebar note about merge tool ─────────
    st.subheader("🧩 Need to merge more Excel files?")
    st.markdown("Use the **Excel Merge Tool** from the sidebar.")
    if st.button("🔀 Go to Excel Merge Tool"):
        st.markdown(
            "👉 Please use the sidebar to switch to **Excel Merge Tool**",
            help="Find '🧩 Merge Tool' in the left-hand navigation."
        )
